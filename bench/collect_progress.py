...
the table below, and every table cell is upserted into output/progress.csv,
so all claims in output/progress.png are data rows. Re-running replaces rows
...
Frozen provenance (all oracle EQUAL, Apple M2, 3,000-file corpus):
...
# (median_ms, source, sdate). Oracle EQUAL on every cell.
FROZEN = {
    "sel-146": {
